...
def re_script(txt, node, sheet, sheet_it):
    ...
    for i in a:
        ...
        if item in mynode_data["in"]:
            ...
            itemval = re_attribute(
                str(mynode_data["in"][item]), node, str("node"), sheet, sheet_it
            )
            ...
            if os.path.exists(shot_init_file):
                ...
                print(
                    " Attr Not Fount in GROUP _init_ File --> Find in PROJ _init_ file"
                )
                init = dnk_init.Cooper_Globals()
                cu_proj_path = init["cooper_proj"] + "/"
                cu_proj_path = cu_proj_path.replace("//", "/")
                prj_path = (
                    cu_proj_path + node_path.replace(cu_proj_path, "").split("/")[0]
                )
                prj_ini_node = prj_path + "/_init_"
                ##########################################################################
                itemval = re_attribute(
                    str(i), prj_ini_node, str("dropnode"), sheet, sheet_it
                )
            ...
            if re.search(":", item):
                ...
                if os.path.exists(one_node):
                    ...
                    if one_node_knob in one_node_data["in"]:
                        ...
                        itemval = re_attribute(
                            str(one_node_data["in"][one_node_knob]),
                            one_node,
                            str("node"),
                            sheet,
                            sheet_it,
            ...
            if mode == "node":
                ...
                if os.path.exists(shot_init_file):
                    ...
                    if item in shot_node_data["in"]:
                        ...
                        prj_path = (
                            cu_proj_path
                            + node_path.replace(cu_proj_path, "").split("/")[0]
                        )
                        prj_ini_node = prj_path + "/_init_"
                        #########################################################################
                        itemval = re_attribute(
